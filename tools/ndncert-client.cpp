...
{
public:
  Client(ndn::Face& face, ndn::Name identity)
    : m_face(face)
...
//      *Request::genCaProfileDiscoveryInterest(caProfile->caPrefix),
...
//    }
  ...
  void
  setConfig(const std::string& filename)
  {
    ProfileStorage profileStorage;
    try {
      profileStorage.load(filename);
    }
    ...
    for (const auto& item : profileStorage.getKnownProfiles()) {
      if (item.caPrefix.isPrefixOf(m_identity)) {
        m_caProfile = item;
        break;
      }
    }
    ...
    if (!m_caProfile) {
      NDN_THROW(std::runtime_error("Could not find a valid authority for the identity `" + m_identity.toUri() + "`"));
      return;
    }
  }
...
private:
  ...
  std::optional<CaProfile> m_caProfile;
};
...
//  terminateSignals.async_wait(handleSignal);
  ...
  description.add_options()
    ...
    ("identity", po::value<ndn::Name>(&identity), "requested identity name to be certified")
    ;
  ...
  ndncert::requester::Client ndncertClient(face, identity);
  ndncertClient.setConfig(configFilePath);
